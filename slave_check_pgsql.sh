...
#check vip status
echo -e "\n"|telnet $masterip 5432 2> tmpstatus
masterstatus=`grep refused tmpstatus|wc -l`
...
function backuptomaster() {
    sudo su - postgres -c "PGPASSWORD=$replicPassword /usr/pgsql-10/bin/pg_basebackup -D $PGDATA -Fp -Xs -v -P -h $masterip -U $replicUser"
    sudo su - postgres -c "sed 's/#hot_standby = on/hot_standby = on/g' -i $PGDATA/postgresql.conf"
    if [[ $? != 0 ]] ; then
        echo "`date` backuptomaster PGDATA failed" >> /etc/keepalived/log/postgresql_keep.log
        exit 1
    fi
...
if [[ $pgsqlstatus != "running" && $masterstatus == 0 ]] ; then
    removedata
    backuptomaster
    addrecover
    ...
    exit 1
elif [[ $pgsqlstatus == "running" && $masterstatus == 1 ]] ; then
...
elif [[ $pgsqlstatus == "running" && $masterstatus == 0 ]]; then
    if [ $viplocation == 1 ] ; then
        status=`sudo su - postgres -c "psql -At -c 'select  pg_is_in_recovery();' "`
        if [ $status == "t" ] ; then
            upgrademaster
    ...
    echo "nothing to do" >> /etc/keepalived/log/postgresql_keep.log
